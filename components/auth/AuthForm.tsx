        ...
        {mode !== 'forgot' && (
          <>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              } label="Apple" onClick={() => handleSocialLogin('apple')} disabled={loading} />
            </div>
          </>
        )}
...
function SocialButton({ icon, label, onClick, disabled }: { icon: React.ReactNode; label: string; onClick: () => void; disabled?: boolean }) {
  return (
    <button type="button" onClick={onClick} disabled={disabled}
      className="flex items-center justify-center gap-2 py-3 rounded-xl border border-white/10 hover:border-white/30 bg-black text-white/80 hover:text-white text-[10px] font-bold uppercase tracking-wider transition cursor-pointer disabled:opacity-50">
      {icon} {label}
    </button>
  );
}
